...
static void _configure(int argc, char *argv[]) {
  ...
  if (!g_env.config_file) {
    static const char *cf[] = { "brocha.ini", "greenrooms.ini" };
  ...
  if (g_env.start_flags & GRSTART_FLAG_USE_AUTO_IP) {
    ...
    if (ip) {
      ...
      free(ip);
      RCGO(rc, exit);
      iwlog_info("Autodetected external IP address for brocha server: %s", g_env.auto_ip);
    } else {
      g_env.auto_ip = "127.0.0.1";
      iwlog_warn("Failed to detect network IP address of brocha server, fallback to: %s", g_env.auto_ip);
    }
  }
  ...
  if (!under_proxy && !(g_env.domain_name || (g_env.cert_file && g_env.cert_key_file))) {
    usage_err
      = "HTTPS support with real domain name must be enabled for your Brocha server."
        ...
        "\n Please check the brocha.pdf manual";
    goto usage;
  }
...
static iwrc _db_open(int argc, char *argv[]) {
  ...
  if (f) {
    ...
    iwxstr_clear(xp);
    RCC(rc, finish, iwxstr_printf(xp, "%s/%s", data_dir, "brocha.db"));
  }
...
static void _on_signal(int signo) {
  ...
  if (sig_cnt == 1) {
    ...
    iwlog_warn("Next SIGINT/SIGTERM will kill brocha by SIGKILL...");
  } else if (sig_cnt >= 3) {
    iwlog_warn("Killing brocha by SIGKILL");
    raise(SIGKILL);
  }
}
